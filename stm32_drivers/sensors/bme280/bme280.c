...
bme_err_t bme280_configure(BME280_t *bme280_dev)
{
	uint8_t config_buff[3] = {
			...
			BME280_CTRL_MEAS(BME280_OSRS_X1,
			                 BME280_OSRS_X1,
			                 BME280_MODE_NORMAL),
			BME280_CONFIG(BME280_STBY_0_5_MS,
						  BME280_FILTER_OFF)
	};

	if(bme280_dev->write(bme280_dev, REG_CTRL_HUM, &config_buff[0], 1) != BME_OK)
		return BME_FAIL;

	if(bme280_dev->write(bme280_dev, REG_CTRL_MEAS, &config_buff[1], 1) != BME_OK)
		return BME_FAIL;

	if(bme280_dev->write(bme280_dev, REG_CONFIG, &config_buff[2], 1) != BME_OK)
		return BME_FAIL;
...
static bme_err_t bme280_read_spi(BME280_t *bme280_dev, uint8_t reg, uint8_t *data_buff, uint16_t len)
{
...
static bme_err_t bme280_write_spi(BME280_t *bme280_dev, uint8_t reg, const uint8_t *data_buff, uint16_t len)
{
...
bme_err_t bme280_read_temp(BME280_t *bme280_dev)
{
	...
	bme280_dev->raw_temperature = ((int32_t)bme280_dev->temp_buff[0] << 12) |
								  ((int32_t)bme280_dev->temp_buff[1] << 4)  |
								  ((int32_t)bme280_dev->temp_buff[2] >> 4);

	bme280_dev->comp_temperature = BME280_compensate_T_int32(bme280_dev, bme280_dev->raw_temperature);
...
bme_err_t bme280_read_press(BME280_t *bme280_dev)
{
	...
	bme280_dev->raw_pressure = ((int32_t)bme280_dev->press_buff[0] << 12) |
							   ((int32_t)bme280_dev->press_buff[1] << 4)  |
							   ((int32_t)bme280_dev->press_buff[2] >> 4);

	bme280_dev->comp_pressure = BME280_compensate_P_int64(bme280_dev, bme280_dev->raw_pressure);
...
bme_err_t bme280_read_hum(BME280_t *bme280_dev)
{
	...
	bme280_dev->raw_humidity = ((int32_t)bme280_dev->hum_buff[0] << 12) |
							   ((int32_t)bme280_dev->hum_buff[1] << 4)  |
							   ((int32_t)bme280_dev->hum_buff[2] >> 4);

	bme280_dev->comp_humidity = bme280_compensate_H_int32(bme280_dev, bme280_dev->raw_humidity);
